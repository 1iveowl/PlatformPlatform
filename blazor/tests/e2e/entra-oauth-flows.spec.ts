...
// Runs only when this deployment enables Entra, read from the bootstrap's system feature flags through the page; otherwise
...
// API's mock provider, so this is mock provider evidence, never a real Entra integration result.
test.beforeEach(async ({ page }) => {
  await requireProviderOrExpectUnavailable(page, "Entra");
});

test.describe("@smoke", () => {
  /**
   * Entra signup and login through the Blazor buttons, in the culture of the running project:
   * - Signup creates a tenant and user and lands in the workspace after the account step of the welcome setup
   * - Logout and login with Entra resolve the same account
  ...
  test("should sign up, log in, redirect an existing user's signup to login and log in with a changed provider email", async ({ page }) => {
    ...
    await runOAuthSignupAndLoginJourney(page, "Entra");
  });
});

test.describe("@comprehensive", () => {
  /**
   * Entra preferred tenant selection and error paths with error page rendering:
   * - The preferred-tenant cookie a tenant switch writes is carried to the Entra login start and selects the signed-in tenant; a malformed value is dropped
   ...
   */
  test("should carry the preferred tenant and render the error paths of Entra flows", async ({ page }) => {
    createTestContext(page);

    await runOAuthPreferredTenantAndRefusalJourney(page, "Entra");
  });
});
