...
import { expectUsersListLoaded, gotoUsersPage, openUserActions, userRow } from "@blazor/e2e/users";
  ...
  test.describe("@comprehensive", () => {
    ...
    test(`should handle tenant switching, logout and the tenant preference in ${culture.locale}`, async ({ page, browser }) => {
      ...
      await step("Switch to the primary tenant while the preference module fails to load & verify a new document on it")(async () => {
        ...
        await switchTenantButton(page, primaryTenantId).click();
      ...
      await step("Switch tenant while an old tenant's profile read and the preference module are held & verify the old surface neither repopulates nor mutates")(async () => {
        ...
        const menu = await openUserActions(page, userEmail, culture.userActions);
      ...
      await step("Click logout and switch account in the same task & verify only the logout is sent and no session remains")(async () => {
        ...
        await page.evaluate(
          ({ primaryTenantId }) => {
            (document.querySelector('[data-testid="logout"]') as HTMLButtonElement).click();
            (document.querySelector(`[data-testid="switch-tenant"][data-tenant-id="${primaryTenantId}"]`) as HTMLButtonElement).click();
          },
          { primaryTenantId }
        );
