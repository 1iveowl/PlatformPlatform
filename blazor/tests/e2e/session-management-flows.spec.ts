...
import { getCurrentSessionIdThroughAccountApi, getSessionsThroughAccountApi, revokeSessionThroughAccountApi } from "@blazor/e2e/account-api";
...
for (const culture of blazorCultures) {
  test.describe("@comprehensive", () => {
    ...
    /**
     * Logout, a missing access token and a revoked session on the Blazor workspace.
     * - Logout lands on the login page, leaves only the antiforgery cookie and the authenticated home then redirects to login
     ...
     */
    test(`should handle logout, access token refresh and session revocation in ${culture.locale}`, async ({ page, browser }) => {
      ...
      // === LOGOUT ===
      await step("Sign up and log out & verify the login page and only the antiforgery cookie remain")(async () => {
