...
// One entry of the user menu. Href is set for a link, Tenant for a tenant of the switch account group.
public sealed record UserMenuItem(UserMenuItemKind Kind, string? Href = null, TenantSwitcherOption? Tenant = null, bool Disabled = false);

// The items of the shell's user menu, matching the React edition's UserMenuDropdownContent: Profile, Preferences, the
// switch account group for a user with more than one tenant (the current tenant marked and not selectable), Account
...
{
    public static IReadOnlyList<UserMenuItem> Create(string? role, IReadOnlyCollection<TenantSwitcherOption> tenants, bool isTransitionBusy)
    {
