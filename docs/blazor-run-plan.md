...
| C Foundation | 17: C1a, C1b, C2, C3a, C3b, C3 review, C4, C4 forms, C4b, C5, C6, C7, C8, V1, V2, V3, stage C review | R2: all of them | none | B6 done |
...
59 planned sessions or task sets after B5 (B6; 17 in C; 9 in D; 8 in E; 6 in F; 6 in G; 5 in H; 4 in N; U1, X1 and X2), 67 to 76 with the follow-up and split allowance of section 8. Stage B is done except B5 and B6; stage C was fixed by B5 on 2026-09-13; stages D to H and N are fixed at session level here and become projects at B6.
...
### Stage C: Foundation. 17 sessions on the next `experiment/` branch, own project.

Project and branch: project `blazor-foundation`, branch `experiment/07-blazor-foundation`, to be taken from the tip of `experiment/06-blazor` when the first session starts (stage B does not merge to `main`, owner decision 2026-09-13). Filled in by B6 on 2026-09-14. Issues are numbered T001 to T017, with T007b (EP-96) inserted between T007 and T008 without renumbering; T001 to T016 including T007b are the 17 planned chain sessions. T017 is the RC2 hop, owner-triggered and outside the chain order. [EP-93](https://linear.app/etarapartners/issue/EP-93/open-clarifications) (Open clarifications) must be answered before the chain reaches C3b.

Delivers: the Blazor edition as a running application behind the gateway with three complete user journeys, the shared contracts and typed client, bootstrap-based identity in the account API whose reads work for a bearer-only client, feature flags and localization from C#, the developer CLI and CI covering it, the public-client protocol map, and the first corpus validated on those journeys. Aligns with the goal by making the core real for the web and preparing it for native clients: reads work without cookies, while cookie-free writes, tenant switching and replacement credentials are proved in C8 and built in stage N, so stage C is not fully native-ready (owner decision 2026-09-14, recorded on [EP-93](https://linear.app/etarapartners/issue/EP-93/open-clarifications)). After stage C a user can sign up, log in, manage users and switch tenants in Blazor, and the workflow that builds the remaining surfaces has been measured. Fixed by B5 on 2026-09-13 from the B4 verdict (the comment on the project, everything checked at `4c5a7dc78`); "correction n" below is B4's numbering.

| Session | Issue | Slice | Delivers | Model, effort |
| -- | -- | -- | -- | -- |
| C1a | [EP-78](https://linear.app/etarapartners/issue/EP-78) | Shell and policy, productionised from the B1 and B2 arrangement | Static SSR shell with per-page render mode following `docs/blazor-render-mode-rule.md`; the FluentUI pin `5.0.0-preview.26254.1` kept with the `nuget.config` source mapping, the note that SemVer orders it below `rc.5` so the upgrade-packages skill must not move it, the exit criterion (a [nuget.org](http://nuget.org) release containing `c813a4a6b`) and the NU3042 decision recorded (correction 1); `base-uri 'none'` kept with the `document.baseURI` shim as production code, every rendered URL root-absolute through one helper, no path base as a literal in components, and a deeper-route interactive test in Chromium, Firefox and Playwright WebKit (2); brand tokens in an external stylesheet, no nonced inline element in `<head>`, and a test that one enhanced navigation produces 0 violations in Chromium (3); charts left out until stage G, the chart package and its nonce shim removed (owner decision 2026-09-13 at B6; correction 4); `form-action 'self'` and `worker-src` evaluated, with the external-login redirects tested before `form-action` is kept (6); the spike-only `csp-variant`, `b1-hooks` and `b1-apex-nonce` switches and the stale `loginReturnPath` harness case deleted, not ported (14); the PWA manifest, icons and standalone window with an installability audit ([EP-59](https://linear.app/etarapartners/issue/EP-59/multi-client-readiness-platform-neutral-client-layer-and-a-native) item 5, first half); no per-user data in the shell, so the offline shell of stage E stays practical | Opus 5, high |
| C1b | [EP-79](https://linear.app/etarapartners/issue/EP-79) | Public-page budget and trimming | The six public pages measured on a trimmed Release publish with Brotli through the gateway, medians of 7 samples with the TTI event defined (12); a public-page budget set, and the cost of keeping FluentUI's JS initializer (455,973 bytes uncompressed) and the scoped CSS bundle (171,567 bytes) off the public surface priced (7); `ILLink.Descriptors.xml` kept, a trimmed-publish smoke test that fires at least one FluentUI event, Brotli kept publish-only until X1 re-checks the Development static-asset defect (8); the Firefox warm load that is no faster than cold (B1 finding 5) explained or recorded as open | Opus 5, high |
| C2 | [EP-80](https://linear.app/etarapartners/issue/EP-80) | Contracts extraction | Platform-neutral `net10.0` contracts and client projects under `application/`, referenced across the build-root boundary, with an architecture test on the transitive dependency boundary that fails the build on a reference to a UI framework or the web host packages; the commands, responses, ids and the problem-details error shape that the three journeys need moved out of `Account.Core` and `SharedKernel`; the rule that every later slice extracts the contracts it needs the same way, so the extraction is never a phase of its own ([EP-59](https://linear.app/etarapartners/issue/EP-59/multi-client-readiness-platform-neutral-client-layer-and-a-native) item 1, with C4) | Opus 5, high |
| C3a | [EP-81](https://linear.app/etarapartners/issue/EP-81) | Production authentication, keys and proxies | The direct user-secrets read and the `Localhost` issuer and audience replaced by SharedKernel's token signing service and validation parameters (verified at `4c5a7dc78`: the APIs validate through `SharedDependencyConfiguration.GetTokenSigningService().GetTokenValidationParameters(...)`); the host joins the cross-service data protection key ring (the local application name now, the Container Apps environment recorded for E3); known proxies configured for forwarded headers; the host recorded as a confidential client of the account API, and the call path, through the gateway or direct, decided and written down (5) | Opus 5, high |
| C3b | [EP-82](https://linear.app/etarapartners/issue/EP-82) | Bootstrap endpoint and authentication state | Identity, runtime configuration and system-scope flags from one endpoint that no client reads from injected HTML ([EP-59](https://linear.app/etarapartners/issue/EP-59/multi-client-readiness-platform-neutral-client-layer-and-a-native) item 2); authentication state across both render modes; logout, expiry and tenant switch; the bootstrap endpoint in the account API and the authenticated resource reads proven by a test to work with a bearer token and no cookie, cookie-free writes deferred to C8 and stage N and browser writes keeping their protections (owner decision 2026-09-14); session state designed on the "unify at the authorization server, not at the credential store" rule ([EP-59](https://linear.app/etarapartners/issue/EP-59/multi-client-readiness-platform-neutral-client-layer-and-a-native) item 3, design constraint); the access-token lifetime recorded as the revocation bound (11) | Opus 5, high |
| C3 review | [EP-83](https://linear.app/etarapartners/issue/EP-83) | Identity boundary review of C3a and C3b | Findings on the C3 issues, blocking or not; the revocation bound and the call path judged | Fable 5.1, xhigh |
| C4 | [EP-84](https://linear.app/etarapartners/issue/EP-84) | Typed clients, transport, host headers and flags | Four portable typed account API clients; structured results, locale/antiforgery/401 handling and feature flag state; host per-request credential adapter with refresh-required signal forwarded to the gateway and removed before the browser; mechanical Users/static-form call migration; portable bulk-delete request mapped to the existing server command, preserving JSON/OpenAPI compatibility; deletion UX remains in D3 | Opus 5, high |
| C4 forms | [EP-96](https://linear.app/etarapartners/issue/EP-96) | T007b: forms, errors and unsaved changes | Shared EditContext/ValidationMessageStore mapper; interactive toast and accessible static-form errors; dirty-dialog and navigation guard; committed maintained Development-only fixtures and browser tests, with a non-Development 404 check; UI string inventory for C5 | Opus 5, high |
| C4b | [EP-85](https://linear.app/etarapartners/issue/EP-85) | List foundation | One shared QuickGrid wrapper (9): the selection model with select-all, indeterminate state and clearing on page, sort or filter change; the row keyboard model; list URL state reconciled with QuickGrid's `?sort=`, `?direction=` and `?page=` through NavigationManager; a localized paginator; a server-page cache; the URL-state model settled here and validated in V2 before any list surface is built | Opus 5, high |
| C5 | [EP-86](https://linear.app/etarapartners/issue/EP-86) | Localization | Shared resource assembly referenced by the host and the client ([EP-59](https://linear.app/etarapartners/issue/EP-59/multi-client-readiness-platform-neutral-client-layer-and-a-native) item 4); culture preserved across prerender; FluentUI's built-in texts through its localizer; the strings of the three journeys in en-US and da-DK | Opus 5, high |
| C6 | [EP-87](https://linear.app/etarapartners/issue/EP-87) | Developer CLI and CI | `pp build`, `test`, `format` and `lint` cover the Blazor root, with the `--all-files` question for `format` decided (B0b finding 4); `e2e` can target the edition; a CI job builds, publishes trimmed and runs the smoke test from C1b (8); the Aspire resource kept | Opus 5, high |
| C7 | [EP-88](https://linear.app/etarapartners/issue/EP-88) | Corpus seed | Rules written from the C1 to C6 code: structure, forms and validation, API access, flags, localization, CSP, render modes, and "no business logic in Razor components" with an example ([EP-59](https://linear.app/etarapartners/issue/EP-59/multi-client-readiness-platform-neutral-client-layer-and-a-native) item 6); the stage B rules (10): render mode on the hosted component, never the router; host-container registration for everything prerendered; absolute hrefs through the helper; no `style-src-attr 'unsafe-inline'`, and components that write style attributes wrapped or avoided; FluentMenu items inside `FluentMenuList`; no `@onkeydown` on a large surface; plain delegates around provider calls; medians of repeated samples for any budget (12); the skills the three journeys need | Fable 5.1, high |
| C8 | [EP-60](https://linear.app/etarapartners/issue/EP-60/c8-spike-prove-the-public-client-authorization-boundary-and-session) | Public-client authorization spike, after C3 | Protocol map naming the authorization server, clients, audiences and scopes; a minimal harness doing S256 PKCE through the system browser against the C3 design; the negative cases [EP-60](https://linear.app/etarapartners/issue/EP-60/c8-spike-prove-the-public-client-authorization-boundary-and-session) lists; the exact backend and gateway changes stage N will need, estimated in sessions, separately from discovery; the writes C3b deferred performed without cookies, with usable replacement credentials after tenant switch, and a verdict on whether stage N can implement that lifecycle practically, without which stage C is not accepted (owner decision 2026-09-14) | Opus 5, high |
| V1 | [EP-89](https://linear.app/etarapartners/issue/EP-89) | Vertical slice: login, signup, one-time password, welcome | The B2 static SSR forms as production code with localized form validation and API error messages kept in English in both cultures; the resend-code form B2 left out; the plain-input one-time password (the six-slot input is stage D); the `@smoke` flows of `login-flows`, `signup-flows` and `landing-page-flows` re-pointed | Opus 5, high |
| V2 | [EP-90](https://linear.app/etarapartners/issue/EP-90) | Vertical slice: users list with role change and side pane | The B3 QuickGrid list rebuilt on the C4b wrapper against the real API under the nonce-only policy; role change and the side pane; the URL-state model validated; the role-management part of `user-management-flows` re-pointed | Opus 5, high |
| V3 | [EP-91](https://linear.app/etarapartners/issue/EP-91) | Vertical slice: tenant switch, logout, session expiry, profile | The authentication and tenant transition journey on C3b; profile read and edit; the single-tab part of `tenant-switching-flows` and the logout and expiry parts of `session-management-flows` re-pointed | Opus 5, high |
| Stage C review | [EP-92](https://linear.app/etarapartners/issue/EP-92) | Review and corpus validation | Findings per slice, BLOCKED stops and rework recorded, `pp claude-usage` per session against the calibration in section 8, the C8 verdict, whether stage D may assume agent-team throughput; the merge recommendation for `main` | Fable 5.1, xhigh |
...
Stop criterion from [EP-50](https://linear.app/etarapartners/issue/EP-50/decide-is-the-blazor-edition-a-product-to-maintain-or-a-demonstration), made concrete: V1, V2 and V3 are delivered within 5 sessions (3 planned plus 2 follow-ups). A sixth is the stop criterion and goes to the owner.

The C4 split does not count against the five-session allowance for V1 to V3.

C4 split, 2026-09-14: the owner authorized T007b (EP-96), session C4 forms, immediately after C4 and before C4b/C5. T007 retains transport and flags; T007b owns forms, errors, guard and maintained browser fixtures. The original C4 BLOCKED attempt remains part of actual session/cost accounting, not a delivered implementation. The driver must use this updated Issue column and table order; reset EP-84 to Todo after the issue/dependency updates, and resume only from a clean working tree in a fresh driver session.
...
| C | 17 (15 implementation including C4 forms, C7, C8 and V1 to V3; 2 reviews) | 22 to 24 | 1.5 on 14 ordinary implementation sessions; C8 at 1, reviews at 1 each |
...
Stage C allowance after the C4 split: 14 ordinary implementation sessions × 1.5 + C8 at 1 + 2 reviews at 1 = 24. The 22 to 24 range retains a two-session planning band; its lower bound is an estimate, not a new measurement. C8 remains a fixed one-session spike. The split adds one planned session; it does not enlarge the separate V1 to V3 allowance.
...
| -- | -- | -- | -- |
| B remaining | B6 | 1 | |
| C | 22 to 24 in chain R2 | 4 to 6 | about one hour per session; one to two stops per day expected (the C3 review, follow-ups) |
...
| -- | -- | -- | -- |
| Phase 2 foundation, 9 to 13 weeks | draft of 2026-09-11 | host, authentication, typed API, flags, localization, six multi-client items | stage C, 22 to 24 sessions, 4 to 6 working days, plus the EP-59 items whose homes are in E and N |
...
| Reduced starter 3 to 4 months; full v1 7 to 9 months for one engineer; parity 11 to 14 | assessment | human engineer-months | not a planning unit here; the session plan is 67 to 76 sessions or task sets and 6 to 9 calendar weeks under the elapsed assumptions above |
...
| 30 to 60 engineer-days of behaviour work | capability map | component and behaviour work before slices | C4, C4 forms, C4b and the items per task set; not added on top |
