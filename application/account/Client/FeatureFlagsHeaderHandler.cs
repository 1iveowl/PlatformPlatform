...
// identity and antiforgery token of a bootstrap the registrant accepts, never from a read it discards.
public sealed class FeatureFlagsHeaderHandler(FeatureFlagState featureFlagState) : DelegatingHandler
{
    ...
    {
        if (request.RequestUri?.AbsolutePath == AccountApiRoutes.Bootstrap) return await base.SendAsync(request, cancellationToken);
