...
Read `.Codex/reference/constitution.md`. Fetch the Linear document "Constitution" on the team (`list_documents` with `query: "Constitution"`, then `get_document`). Compare the version lines; a mismatch is a HIGH finding on its own. The repository copy is the one used for the checks.
...
- Terms: nouns that name a domain concept, checked against `.Codex/skills/codebase-overview/references/glossary.md`
