...
- Pointers (not contents) to relevant `.Codex/rules/` files.
...
`[PRODUCT_MANAGEMENT_TOOL]` tasks (when chosen): follow `.Codex/reference/product-management/[PRODUCT_MANAGEMENT_TOOL].md`. Each [task]: title (imperative), description (consolidated finding from `SUMMARY.md`), link to parent [feature] if any, [Planned] status, current iteration.
