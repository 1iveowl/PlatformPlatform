...
path-scoped rules in `.Codex/rules/` for detail; this skill holds only what they cannot tell you. Terms: [references/glossary.md](references/glossary.md).
...
| `blazor/` | Blazor edition build root on its own SDK 11 `global.json`, rules in `.Codex/rules/blazor/`; `--blazor` target of build, format, lint, test and e2e |
...
- `.Codex/commands/` no longer exists, but `SyncAiRulesAndWorkflowsCommand.cs` still reads it (a no-op).
...
- A `blazor-host` started before `build --blazor` serves stale framework files and the WebAssembly runtime fails to start;
  ...
  its FluentUI pin must not move: pass `--exclude Microsoft.FluentUI.AspNetCore.Components` (rule `.Codex/rules/blazor/component-library.md`).
