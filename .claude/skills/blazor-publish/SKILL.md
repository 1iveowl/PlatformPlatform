...
- `release-rehearsal` - the release-and-rollback rehearsal of the version policy: `--current <name>` and `--previous <name>` (default `b` and `a`). It needs two publishes (`blazor-publish --folder a --version 0.9.0` and `blazor-publish --folder b`) and no other `blazor-serve`, because it serves each publish in turn itself through `blazor-serve --folder`. It drives two tabs across the switch and back, one that navigates and one that stays on its page: a supported client's write, the rollback's missing fingerprinted assets, the reload prompt on a write from a stale tab, from a tab that never navigated and from a client outside the version window, atomic asset sets, the unsaved-changes dialog on the reload prompt's own action, and recovery on the restored release. See `docs/blazor-recovery-runbook.md`.
