...
5. Keep flag state per identity scope: the state records `UserId` and `TenantId` from the bootstrap, a successful tenant switch ends the previous identity at once and leaves with a full document navigation, and a lost session resets the state. Never cache a flag value in a static field or across identities.
