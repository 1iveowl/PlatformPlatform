...
1. Register the library on both sides with `AddFluentUIComponents(configuration => configuration.Localizer = new FluentResourceLocalizer())`, in `Blazor.Host/HostApplication.cs` for static rendering and prerendering and in `Blazor.Client/Program.cs` for the browser. Use a FluentUI component only on a page that hosts a WebAssembly component: its custom elements are defined by the browser bundle in item 6, which a static page never downloads, so on a static page the component renders as an undefined element with no shadow root and no behaviour. Write the static public surface with plain markup and the classes in `app.css`.
...
5. Keep the host page's `no-fuib-style` attribute on `<html>` and the absolute link to `default-fuib.css`; without it the library's initializer fetches the stylesheet relative to the document URL and gets 404 below the path base. The stylesheet is 914 bytes compressed and stays on every page; the browser bundle is what a static page must not pay for.
6. Keep the library's browser bundle off the static public surface. The library ships that bundle as its JavaScript initializer, and `blazor.web.js` imports every web initializer on every document, which cost each static public page about 105 KB. Two targets in `Blazor.Host.csproj` remove it from this project's JS module manifest, and `blazor/Blazor.Host/wwwroot/js/Blazor.Host.lib.module.js`, the host's own initializer, imports it instead: on a document `App.razor` marked `data-interactive-surface` from `[InteractiveSurface]`, and on a WebAssembly runtime start, which is how an enhanced navigation reaches such a page. Do not replace that initializer with the WebAssembly boot configuration, which lists the library under `resources.libraryInitializers` after a publish but leaves that list empty in a Development build. `HostSecurityTests.StaticPublicPage_WhenRendered_ShouldNameNoComponentLibraryWebInitializer` reads the initializers back out of the rendered document and `public-pages.mjs` fails a public page that requests the bundle at all.
7. Keep `ILLink.Descriptors.xml` in the client project; it preserves `OverflowChangedItem`, the element type of the library's overflow event arguments, whose constructor the trimmer removes so a trimmed publish throws on the event. Add a type to it only when the trimmed smoke test proves the trimmer removed it, and delete an entry once the library preserves the type itself.
8. Match text inside a FluentUI component by containment in tests (`toContainText`); its shadow root adds slots the exact text match does not see.
