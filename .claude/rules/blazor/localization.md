...
4. Put validation messages on the form model with `ErrorMessageResourceType = typeof(CommonStrings)` and `ErrorMessageResourceName = nameof(CommonStrings.Key)`, so no framework default English message reaches a page.
