...
1. Inject the typed client for the area (`EmailAuthenticationClient`, `AuthenticationClient`, `UsersClient`, `TenantsClient`) and call its method with request records from `Account.Contracts` and a required `CancellationToken`: `HttpContext.RequestAborted` in a static form handler, the component's own token or `CancellationToken.None` in the browser.
...
6. Send state-changing calls through the registered chain, which adds the antiforgery token from the latest bootstrap, the UI culture as `X-Locale` and applies the `x-user-feature-flags` response header to `FeatureFlagState`. Never add these headers by hand.
7. Register the clients in the host with `HostAccountApiHandler` (server to server, direct to `ACCOUNT_API_URL`), which relays the current request's bearer token, antiforgery pair, locale and client address on every send and copies the token headers and `Set-Cookie` back. The handler is pooled, so it holds nothing per user; read everything from `HttpContext` inside `SendAsync`.
8. Resolve a client lazily through `IServiceProvider` in a component that uses it only after the runtime starts, so prerendering never constructs it; inject it directly when a static form handler or prerender calls it.
9. Do not retry: a timeout is a `TransportFailure`, and a cancellation by the caller is rethrown as `OperationCanceledException`.
