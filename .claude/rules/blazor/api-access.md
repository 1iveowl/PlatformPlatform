...
11. Leave a client that no longer matches its server to the write gate: `StaleClientRequestHandler`, the outermost handler of the client's chain, answers a mutation locally with `412` instead of sending it once `ClientVersionState` reports this runtime stale. Two signals set that: the version the bootstrap carries, which is the account API's own, leaving the supported window, and `StaleAssetProbe` finding a fingerprinted asset of this document no longer served. Both are read again by `StaleClientRecheck` before a mutation is forwarded, so a tab learns at its next in-app navigation or before its next write, whichever comes first; a re-check that cannot complete never blocks the write, and a runtime already stale asks nothing more. `ApiFailureClassifier` classifies the refusal as `ApiFailureKind.Version`, and the shared presentation turns it into the same reload prompt as an antiforgery rejection: a warning toast with a "Reload page" action on an interactive surface, a form message with `IsReloadRequired` on a form. A component never checks a version, compares one or writes its own message, and a read and a logout are never gated. The policy is `docs/blazor-version-policy.md` and the operator side `docs/blazor-recovery-runbook.md`.
