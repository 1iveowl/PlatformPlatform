...
1. Put the request nonce on every script and stylesheet element the host page renders: the stylesheet links, the `<ImportMap>` and the four scripts in `App.razor` (`js/verification-timer.js` serves the static verification pages, which have no runtime to import a module; `js/theme.js` is a parser-blocking script in `<head>` so the stored theme applies before first paint, which a deferred module cannot guarantee) carry `nonce="@_nonce"`, read with `HostShell.GetNonce(HttpContext)`. Preload links are URL-checked and take no nonce. No other page or component renders a script or stylesheet element; `Development/PolicyProbe.razor` is the one exception, which exists to prove the policy blocks what it should.
