...
1. Put the request nonce on every script and stylesheet element the host page renders: the stylesheet links, the `<ImportMap>` and the two scripts in `App.razor` carry `nonce="@_nonce"`, read with `HostShell.GetNonce(HttpContext)`. Preload links are URL-checked and take no nonce. No other page or component renders a script or stylesheet element; `Development/PolicyProbe.razor` is the one exception, which exists to prove the policy blocks what it should.
